...
from bokeh.plotting import figure, show # Inclusion de librerias necesarias para la ejecucion del proyecto, para que pueda correr el proyecto tuve que instalar previamente el request pip y bokeh pip
...
def solicitar_datos_cliente(partidos):
    ...
    partido_id = input("ID del partido al que desea comprar: ")
...
clientes = [] # Lista de clientes, se guardan todos las instancias del cliente cada vez que compra una entrada /se tiene que registrar manuealmente cualquier cliiente 
...
def realizar_compra_restaurante(cedula, estadios):
                ...
                for restaurant in estadio.restaurants:
                    for product in restaurant.products:
                        if product.name.lower() == nombre_producto.lower():
                            ...
                            if cliente.edad < 18 and product.tipo.lower() == "alcoholic": # Si el cliente es menor de edad, no deja comprar bebidas alcohólicas
    ...
    for producto in productos_seleccionados:
        ...
        producto.stock -= 1 # Restando el producto del stock total del almacen
    ...
    if opcion_pagar == "Si":
        # Realizar el proceso de pago de los productos seleccionados por el cliente previamente
        print("Pago exitoso:")       
        print("Subtotal: $", subtotal)
...
# Funcion para buscar productos de los restaurantes dependiendo del criterio de busqueda (depende de la eleccion de producto)
...
def main():
    ...
    opcion = 0
    print("--> Euro 2024 - Venta de Tickets <--")
    while(opcion != 7): # Ciclo con el flujo principal del programa
        print("Menu: ")
        print("1. Busqueda de partidos.")
